...
final class DownloadTask: NSObject, @unchecked Sendable, URLSessionDownloadDelegate {
    ...
    func start() {
        ...
        guard !isCancelled else {
            completionContinuation?.resume(throwing: SparkleyError.downloadCancelled)
            return
        }
    ...
    nonisolated func urlSession(
        ...
        do {
            ...
            progressContinuation.finish()
            completionContinuation?.resume(throwing: SparkleyError.downloadFailed(url, underlying: error))
        }
    }
